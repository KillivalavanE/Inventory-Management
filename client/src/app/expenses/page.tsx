...
const Expenses = () => {
    ...
    const aggregatedData:AggregatedDataItem[]=useMemo(()=>{
        const filtered:AggregatedData = expenses.filter((data:ExpenseByCategorySummary)=>{
            const matchCategory = selectedCategory==="All" || selectedCategory===data.category
            const dataDate = parseDate(data.date)
            const matchDate = !startDate || !endDate || (dataDate >=startDate && dataDate<=endDate)
    ...
    const classNames = {
        ...
        selectInput:"mt-1 block w-full pl-3 pr-10 py-2 text-base border-red-300 focus:ring-indigo-500 focus:border-indigo-900 sm:text-sm rounded-md"
    }
    if(isLoading){
        ...
        <div>
            ...
            <div className="flex flex-col md:flex-row justify-between gap-4">
                <div className="w-full md:w-1/3 bg-white shadow rounded-lg p-6">
                    <h3 className="text-lg font-semibold mb-4">Filter by Categorty and Date</h3>
                    <div className="space-y-4">
                        <div>
                            <label htmlFor="category" className={classNames.label}>Category</label>
                            <select id="category" name="category" defaultValue="All" className={classNames.selectInput} onChange={(e)=>setSelectedCategory(e.target.value)}>
                                <option>All</option>
                ...
                <div className="flex-grow p-4 bg-white shadow rounded-lg md:p-6">
                        ...
                        <PieChart>
                            <Pie data={aggregatedData} cx="50%" cy="50%" label outerRadius={150} fill="#8884d8" dataKey="amount" onMouseEnter={(_,index)=>setActiveIndex(index)}>
                                {aggregatedData.map((entry:AggregatedDataItem,index:number)=>(
                                    <Cell key={index} fill={index===activeIndex ? "rgb(29,78,216)":entry.color} />
                                ))}
                            </Pie>
                            <Tooltip />
                            <Legend />
